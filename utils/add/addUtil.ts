...
export async function doFilesCopy(pkgs: Package_Schema[], dest: string) {
  ...
  await Promise.resolve(
    pkgs.forEach(async (pkg) => {
      ...
      await tryCatch(fs.mkdir(dir, { recursive: true }));
      ...
      await Promise.resolve(
        pkg.files.forEach(async (file) => {
          ...
          const [, notExists] = await tryCatch(
            fs.access(destination, constants.F_OK),
          );

          if (!notExists)
            return console.log(
              `A file with same, exist in the target location ${destination}. `,
            );

          await tryCatch(fs.cp(source, destination, { recursive: true }));
        }),
      );
    }),
  );
...
export async function fileCopier(selectedPkgs: string[], config: CONFIG_TYPE) {
  ...
  if (invalidPkgs?.length > 0) {
    console.log(
      ` These packages: [ ${invalidPkgs.join(", ")} ] were not found`,
    );
    console.log("");
